...
from casadi import SX, inv, vertcat, horzcat, sin, cos, solve, mtimes
import numpy as np
...
def parallel_axis_theorem(I_cm, m, r, dr_dt, dr_dtheta):
    ...
    d_I_A_dtheta1 = m * ((mtimes(r.T, dr_dtheta[:,0]) + mtimes(dr_dtheta[:,0].T, r)) * SX.eye(3) - mtimes(dr_dtheta[:,0],r.T) - mtimes(r,dr_dtheta[:,0].T))
    # Derivative wrt theta2
    d_I_A_dtheta2 = m * ((mtimes(r.T, dr_dtheta[:,1]) + mtimes(dr_dtheta[:,1].T, r)) * SX.eye(3) - mtimes(dr_dtheta[:,1],r.T) - mtimes(r,dr_dtheta[:,1].T))
    # Derivative wrt theta3
    d_I_A_dtheta3 = m * ((mtimes(r.T, dr_dtheta[:,2]) + mtimes(dr_dtheta[:,2].T, r)) * SX.eye(3) - mtimes(dr_dtheta[:,2],r.T) - mtimes(r,dr_dtheta[:,2].T))
...
def phase_A_model():
    ...
    theta = SX([0.05, 0.4, 0.05])
    theta_dot = SX.zeros(3)
    w = vertcat(r_b, theta)
    ...
    # Explicit dynamics: x_dot = [q_dot; M⁻¹(-C - G + u)]
    ddw = SX.sym('q_ddot', nsd, 1)
    ddw = solve(M_b_tilde, tau - c_b_tilde)

    f_expl = vertcat(r_b_dot, ddw, dq)

    # Implicit dynamics: xdot - f_expl = 0
    f_impl = vertcat(
        dw[:3] - xdot[:3],
        M_b_tilde @ xdot[3:6] - c_b_tilde + tau,
        dq - xdot[6:]
    )
    # xdot - f_expl
